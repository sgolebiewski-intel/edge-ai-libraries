...
- **On-device AI Services**: vLLM-IPEX-serving and multilevel-video-understanding run locally on a single Intel® Panther Lake (PTL) 358H host with shared system RAM — no discrete accelerator or remote inference cluster. A single `Qwen3.5-35B-A3B` model serves both the VLM (captioning) and LLM (aggregation) roles from one OpenAI-compatible endpoint.
