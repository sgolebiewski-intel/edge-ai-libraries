...
| Device Targeting | Not supported | All OpenVINO™ toolkit-supported devices: CPU / GPU / NPU, plus heterogeneous execution via `HETERO:<dev>[,<dev>...]` (e.g. `HETERO:GPU,CPU`) | Configure the target device in the download API configuration, if needed. |
